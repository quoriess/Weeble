...
var tries=0;
function copyToCB(){
	var t1=new Date(2022,5,22);
	var nw=new Date();
	var utct1=new Date(t1.getUTCFullYear(),t1.getUTCMonth(),t1.getUTCDate());
	var utcd=new Date(nw.getUTCFullYear(),nw.getUTCMonth(),nw.getUTCDate())
	var dayNo= (Math.floor((utcd-utct1)/(24*3600*1000))+1);
	navigator.clipboard.writeText("Gamele #"+dayNo+": \n"+"\uD83D\uDFE5".repeat(tries-1)+"\uD83D\uDFE9");
}
function valueEntered() {
    ...
    if(txtbox.value==keyNow){
		...
		$(".hiddenBox").html("You guessed it right! <button style='float:right' onclick='copyToCB()'>Share</button>");
		...
		if (clueNo == 6) {
			...
			$(".hiddenBox").html("You failed! The answer was: <b>"+ keyNow+"</b>");
			$(".hiddenBox").css("display","block");
		}
	}
	txtbox.value = "";
